...
CUSTOM_DTEAL = colors.Color(red=(33.0/255),green=(118.0/255),blue=(210.0/255)) ## This is the color to customize
...
def write_philadelphia_profile_pdf(data_dict, output_pdf_path):
...
#    column_10 = Frame(doc.leftMargin, doc.height-0.125*doc.height, doc.width/3-12, 0.13*doc.height, showBoundary=0)    
    vthep_logo = IMG_PATH+"/city_of_philadelphia_logo.png"
    im = Image(vthep_logo, 2.0*inch, 0.54*inch)
    Story.append(im)
    ...
    # Cost
    y_offset += 0.02
    text_c231, text_c232 = Highlights.cost_box(data_dict, CUSTOM_DTEAL, 'ELECTRIFY')
    ...
    if data_dict['systemDefinedPropertyType'] == 'Multifamily Housing':
        if data_dict['energy_star_score'] < 75.0:
            Story.append(Paragraph('<font name="InterstateLight" color=blue><link href="https://www.peco.com/ways-to-save/for-your-home/multifamily/property-owners">PECO</link></font> and <font name="InterstateLight" color=blue><link href="https://pgwenergysense.com/residential-rebates/">PGW</link></font> offer rebates and discounts on energy efficiency upgrades to reduce your property’s energy costs and carbon footprint', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('<font name="InterstateLight" color=blue><link href="https://philly-pa.beam-portal.org/helpdesk/kb/rebates/125/">Learn more</link></font> about financing options such as low interest loans that are available for energy efficiency, renewable energy and water conservation projects', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('You have the power to choose your electricity supplier! You can search suppliers by price or services offered such as up to 100% renewable energy at <font name="InterstateLight" color=blue><link href="https://www.papowerswitch.com/">PAPowerSwitch.com</link></font>', pc291, bulletText=UNCHECKED.encode('UTF8')))
        else:
            Story.append(Paragraph('Your property’s high energy star score makes it eligible for an optional ENERGY STAR Certification. Learn how to obtain one on the <font name="InterstateLight" color=blue><link href="https://www.energystar.gov/buildings/building-recognition/building-certification">ENERGY STAR website</link></font>', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('<font name="InterstateLight" color=blue><link href="https://www.peco.com/ways-to-save/for-your-home/multifamily/property-owners">PECO</link></font> and <font name="InterstateLight" color=blue><link href="https://pgwenergysense.com/residential-rebates/">PGW</link></font> offer rebates and discounts on energy efficiency upgrades to reduce your property’s energy costs and carbon footprint', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('You have the power to choose your electricity supplier! You can search suppliers by price or services offered such as up to 100% renewable energy at <font name="InterstateLight" color=blue><link href="https://www.papowerswitch.com/">PAPowerSwitch.com</link></font>', pc291, bulletText=UNCHECKED.encode('UTF8')))
    else:
        if data_dict['energy_star_score'] < 75.0:
            Story.append(Paragraph('Large, non-residential properties are required to complete the <font name="InterstateLight" color=blue><link href="https://www.phila.gov/programs/building-energy-programs/building-energy-performance-program/">Building Energy Performance Program</link></font>, also known as Tune-Ups, every five years. Find your next compliance deadline here', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('<font name="InterstateLight" color=blue><link href="https://www.peco.com/ways-to-save/for-your-home/multifamily/property-owners">PECO</link></font> and <font name="InterstateLight" color=blue><link href="https://pgwenergysense.com/residential-rebates/">PGW</link></font> offer rebates and discounts on energy efficiency upgrades to reduce your property’s energy costs and carbon footprint', pc291, bulletText=UNCHECKED.encode('UTF8')))
            Story.append(Paragraph('<font name="InterstateLight" color=blue><link href="https://philly-pa.beam-portal.org/helpdesk/kb/rebates/125/">Learn more</link></font> about financing options such as low interest loans that are available for energy efficiency, renewable energy and water conservation projects', pc291, bulletText=UNCHECKED.encode('UTF8')))
        else:
            Story.append(Paragraph('Large, non-residential properties are required to complete the <font name="InterstateLight" color=blue><link href="https://www.phila.gov/programs/building-energy-programs/building-energy-performance-program/">Building Energy Performance Program</link></font>, also known as Tune-Ups, every five years. Find your next compliance deadline here', pc291, bulletText=UNCHECKED.encode('UTF8')))
...
# Run with:  python3 -m label.populate_beam_philadelphia
if __name__ == '__main__':
 #   data_dict = {'street': '77 MASSACHUSETTS AVE', 'city': 'CAMBRIGE', 'state': 'MA', 'zipcode': '02139', 'year_built': 1895, 'year_ending': 2022, 'propGrossFloorArea': 10000.0, 'systemDefinedPropertyType': 'Office', 'energy_star_score': 50, 'site_total': 3434,  'siteIntensity': 73.60, 'medianSiteIntensity': 176.40, 'percentBetterThanSiteIntensityMedian': 0.25, 'cons_mmbtu_min': 0, 'siteEnergyUseElectricityGridPurchase': 1000.0, 'siteEnergyUseElectricityGridPurchaseKwh': 100000.0, 'siteEnergyUseNaturalGas': 1000.0, 'siteEnergyUseKerosene': 0.0, 'siteEnergyUsePropane': 1000.0, 'siteEnergyUseDiesel': 0.0, 'siteEnergyUseFuelOil1': 0.0, 'siteEnergyUseFuelOil2': 0.0, 'siteEnergyUseFuelOil4': 0.0, 'siteEnergyUseFuelOil5And6': 0.0, 'siteEnergyUseWood': 0.0, 'energyCost': 10000.0, 'energyCostElectricityOnsiteSolarWind': 2110.0, 'energyCostElectricityGridPurchase': 1000.0, 'energyCostNaturalGas': 1000.0, 'energyCostKerosene': 0.0, 'energyCostPropane': 1000.0, 'energyCostDiesel': 0.0, 'energyCostFuelOil1': 0.0, 'energyCostFuelOil2': 0.0, 'energyCostFuelOil4': 0.0, 'energyCostFuelOil5And6': 0.0, 'energyCostWood': 0.0, 'cons_solar': -11000.0, 'estar_wh': True, 'yoy_percent_change_site_eui': None, 'yoy_percent_change_elec': -0.1, 'totalLocationBasedGHGEmissions': 150, 'onSiteRenewableSystemGeneration': 20000, 'numberOfLevelOneEvChargingStations': 3, 'numberOfLevelTwoEvChargingStations': 0, 'numberOfDcFastEvChargingStations': 0}
    ...
    "yoy_percent_change_site_eui": 20,
    "zipcode": "04101"
}    

    out_file = 'Philadelphia_BEAM_Profile.pdf'
    write_philadelphia_profile_pdf(data_dict, out_file)
